...
export default async function EventsPage({
  // Renamed component to EventsPage
  searchParams,
}: {
  ...
  console.log(
    "EventsPage (Server): Read token from cookie - ", // Updated log message
  ...
  try {
    ...
    console.error("EventsPage: Failed to prefetch calendars on server:", error); // Updated log message
  ...
  if (calendarIdToFetch !== undefined) {
    try {
      ...
      console.error(
        `EventsPage: Failed to prefetch events for calendar ${calendarIdToFetch} on server:`, // Updated log message
        error
      );
